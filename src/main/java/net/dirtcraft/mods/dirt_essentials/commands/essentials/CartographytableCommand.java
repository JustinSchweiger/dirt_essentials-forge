...
import net.dirtcraft.mods.dirt_essentials.menus.CartographyTable;
...
public class CartographytableCommand {
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		LiteralArgumentBuilder<CommandSourceStack> commandBuilder = Commands
				.literal("cartographytable")
				.requires(source -> PermissionHandler.hasPermission(source, EssentialsPermissions.CARTOGRAPHYTABLE))
				.executes(CartographytableCommand::execute);
	...
	private static int execute(CommandContext<CommandSourceStack> commandSourceStackCommandContext) throws CommandSyntaxException {
		...
		player.openMenu(new MenuProvider() {
			...
			public @NotNull Component getDisplayName() {
				return new TextComponent("§cCartography Table");
			}
			...
			public @NotNull AbstractContainerMenu createMenu(int pContainerId, @NotNull Inventory pPlayerInventory, @NotNull Player pPlayer) {
				return new CartographyTable(pContainerId, pPlayerInventory, ContainerLevelAccess.create(player.level, player.blockPosition()));
			}
		});
