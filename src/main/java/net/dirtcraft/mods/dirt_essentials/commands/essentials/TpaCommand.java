...
public class TpaCommand {
	public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
		dispatcher.register(Commands
				.literal("tpa")
				.requires(source -> PermissionHandler.hasPermission(source, EssentialsPermissions.TPA))
	...
	private static int tpa(CommandSourceStack source, ServerPlayer target) throws CommandSyntaxException {
		...
		if (TeleportManager.hasTeleportsDisabled(target.getUUID())) {
			source.sendFailure(new TextComponent(Strings.ESSENTIALS_PREFIX + "§cYou cannot teleport to §6" + target.getName().getString() + " §cbecause this player has teleports disabled!"));
			return Command.SINGLE_SUCCESS;
		}

		if (TeleportManager.hasOutgoingRequestToTarget(player.getUUID(), target.getUUID(), TeleportRequest.TeleportType.TPA)) {
		...
		TextComponent targetComponent = new TextComponent("§d" + player.getGameProfile().getName() + " §7has requested to teleport to you.");
		...
		TeleportRequest outgoingRequest = new TeleportRequest(player.getUUID(), target.getUUID(), LocalDateTime.now(), TeleportRequest.Type.OUTGOING, TeleportRequest.TeleportType.TPA);
		TeleportRequest incomingRequest = new TeleportRequest(player.getUUID(), target.getUUID(), LocalDateTime.now(), TeleportRequest.Type.INCOMING, TeleportRequest.TeleportType.TPA);
